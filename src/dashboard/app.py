...
# App Header
st.title("Job Market Intelligence & Application Tracker")
st.markdown("<p style='color: #5f6368; font-size: 1.1rem; margin-top: -10px;'>Local-first analytics tracking multi-source roles and personal application pipeline.</p>", unsafe_allow_html=True)
st.markdown("---")

# Navigation Tabs (No icons)
tab1, tab2 = st.tabs(["Market Intelligence", "My Applications Pipeline"])
...
# =========================================================
# TAB 2: MY APPLICATIONS PIPELINE
# =========================================================
with tab2:
    st.subheader("My Tracked Job Applications")
    st.markdown("Jobs captured via your Plasmo browser extension while browsing JobStreet or MYFutureJobs.")
    ...
    try:
        ...
        if "job_applications" in table_names:
            df_apps = con.execute("SELECT * FROM job_applications ORDER BY applied_at DESC").fetchdf()
            con.close()
            
            if not df_apps.empty:
                col1, col2, col3 = st.columns(3)
                col1.metric("Total Applications Tracked", len(df_apps))
                col2.metric("Platforms Used", df_apps["source_platform"].nunique())
                col3.metric("Active Pipeline", len(df_apps[df_apps["status"] != "Rejected"]))
                
                st.markdown("---")
                st.dataframe(df_apps, use_container_width=True, hide_index=True)
            else:
                st.info("No applications tracked yet! Install your Plasmo extension, browse JobStreet or MYFutureJobs, and click 'Track Application'.")
        else:
            st.warning("The job_applications table hasn't been initialized yet. Run connection.py to create it.")
            con.close()
